...
// LayoutConfigFile: reads, validates and resolves one orientation's layout
// config file into a flat, render-ready model.
//
// Single responsibility: read + validate + resolve one file. It does not
// render anything and does not know about BarContent.qml's layout code.
Item {
    id: root

    // ------------------------------------------------------------------
    // Identity - supplied by BarTweaks per orientation
    // ------------------------------------------------------------------

    required property string configPath
    required property string configFolder
    ...
    FileView {
        ...
        onLoadFailed: {
            ...
            if (error === FileViewError.FileNotFound) {
                console.log("BarTweaks: " + root.configPath + " not found, creating default...");
                ensureConfigDirProcess.running = true;
            } else {
                console.warn("BarTweaks: failed to load " + root.configPath + ":", FileViewError.toString(error));
                root.fileSettled = true;
            }
        }
    ...
    function parseConfig(text) {
        ...
        try {
            ...
            if (!parsed || typeof parsed !== "object") {
                console.warn("BarTweaks: " + root.configPath + " is not a JSON object");
                return null;
            }
            return parsed;
        } catch (e) {
            console.warn("BarTweaks: malformed " + root.configPath + ":", e);
            return null;
        }
    }
    ...
    // Whether this orientation should render: config valid, the `layout`
    // section exists, and the integrated dock (out of scope) isn't active.
